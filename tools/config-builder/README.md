...
committer:
  database:
    ...
    table_pre_split_tablets: 8         # Yugabyte only
    tls:
      enabled: true
...
the generated database config points to that path.
...
When `tls.enabled` is true, non-database committer components also receive peer
TLS identities under the first peer organization by default. The generated local
layout mirrors the Ansible role: each committer component mounts `config/tls`,
the coordinator receives verifier/validator CA files, and the sidecar receives
orderer/coordinator CA files for outbound TLS connections.
