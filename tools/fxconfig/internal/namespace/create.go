...
func setupMSP(mspCfg MSPConfig) (msp.MSP, error) { //nolint:ireturn
	...
	// Check if BCCSP config is provided
	if mspCfg.BCCSPConfig != nil {
		// Use factory to initialize BCCSP based on configuration
		cp, err = initBCCSP(mspCfg.BCCSPConfig)
		if err != nil {
			return nil, fmt.Errorf("failed to initialize BCCSP: %w", err)
		}
	} else {
		// Fallback to software-based BCCSP (original behavior)
...
// initBCCSP initializes BCCSP based on the provided configuration
func initBCCSP(config *BCCSPConfig) (bccsp.BCCSP, error) {
	if config.PKCS11 != nil {
		// PKCS11 configuration
		opts := pkcs11.PKCS11Opts{
			Security:       config.PKCS11.Security,
			Hash:           config.PKCS11.Hash,
			Library:        config.PKCS11.Library,
			Label:          config.PKCS11.Label,
			Pin:            config.PKCS11.Pin,
			SoftwareVerify: config.PKCS11.SoftwareVerify,
			Immutable:      config.PKCS11.Immutable,
		}

		// Create a dummy keystore (PKCS11 doesn't use file-based keystore)
		ks := &dummyKeyStore{}

		// Create PKCS11 BCCSP
		return pkcs11.New(opts, ks)
	} else if config.SW != nil {
		// Software configuration - use default security level
		ks := sw.NewDummyKeyStore()

		return sw.NewWithParams(config.SW.Security, config.SW.Hash, ks)
	}

	return nil, fmt.Errorf("no valid BCCSP configuration provided")
}

// dummyKeyStore is a no-op keystore for PKCS11 mode
// PKCS11 stores keys in the HSM/KMS, not in a file-based keystore
